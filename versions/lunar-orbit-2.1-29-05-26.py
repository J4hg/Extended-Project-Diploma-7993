...
EarthMass = 5.972 * 10**(24) # Earth mass
MoonMass = 7.342 * 10**(22) # Moon mass
...
Velocity = np.array([0, np.sqrt(G * EarthMass / MagPos)])  # This is a variable that stores the vector velocity of the moon.
...
for n in range(0, NumTimeSteps): # Loop that iterates through the physics of the program for NumTimesteps amount
    ...
    Acceleration = - G * EarthMass / MagPos ** (3) * Pos #  Calculation of the magnitude of the acceleration of the moon multiplied by the unit vector of position of the moon. This calculations the vector acceleration of the moon.
    ...
    if iteration == 60:
        ...
        MagVelocity = np.linalg.norm(Velocity)
        KineticEnergy = 0.5 * MoonMass * MagVelocity ** 2
        with open("kinetic_energy_verlet.txt", 'a') as file:
            file.write(f"{KineticEnergy}\n")
        iteration = 0
